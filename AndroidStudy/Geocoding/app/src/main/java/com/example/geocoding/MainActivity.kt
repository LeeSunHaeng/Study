...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var addrBtn = findViewById<Button>(R.id.addrBtn)
        var mapBtn1 = findViewById<Button>(R.id.mapBtn1)
        var latBtn = findViewById<Button>(R.id.latBtn)
        var mapBtn2 = findViewById<Button>(R.id.mapBtn2)
        ...
        addrBtn.setOnClickListener {
            ...
            try{
                ...
                list = geocoder.getFromLocation(d1,d2,10)   //위도 경도를 주소로 변환하는 부분
            }catch(e:IOException){
                Log.d("위도,경도","입출력 오류")
            }
            if(list != null){
